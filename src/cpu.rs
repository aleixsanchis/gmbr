...
impl CPU{
    ...
    fn execute_instruction(&mut self, instruction: u8) -> u8{
        if self.cb_prefix{
            ...
            match instruction {
                ...
                0x80..=0xBF => {let bit = parse_destination_register(instruction, 0x08); self.res_opcode(instruction, bit)},
                _ => {self.print_registers(); handle_unimplemented_instruction(instruction, true); 0}
            }
        }
        else{
            match instruction {
                ...
                // POP DE
                0xD1 => {let value = self.pop_from_stack(); self.registers.setde(value); 3},
                // PUSH DE
                0xD5 => {let value = self.registers.de(); self.push_to_stack(value); 4},
                // LDH (a8), A
                0xE0 => {let address = self.fetch_byte(); self.write_byte(0xFF00 + address as u16, self.registers.a); 3},
                // POP HL
                0xE1 => {let value = self.pop_from_stack(); self.registers.sethl(value); 3},
                // LD (C), A
                0xE2 => {let address = self.registers.c; self.write_byte(0xFF00 + address as u16, self.registers.a); 2},
                // PUSH HL
                0xE5 => {let value = self.registers.hl(); self.push_to_stack(value); 4},
                // AND d8
                0xE6 => {let value = self.fetch_byte(); self.alu_and(value); 2},
                // JP (HL)
                0xE9 => {let address = self.registers.hl(); self.jump_to(address); 1},
                // LD (a16), A
    ...
    fn rlc_opcode(&mut self, opcode: u8) -> u8{
        ...
        self.registers.set_flags(CpuFlags::N, false);
        self.registers.set_flags(CpuFlags::H, false);
        self.registers.set_flags(CpuFlags::C, operand & 0x80 == 0x80);
    ...
    fn rrc_opcode(&mut self, opcode: u8) -> u8{
        ...
        self.registers.set_flags(CpuFlags::N, false);
        self.registers.set_flags(CpuFlags::H, false);
        self.registers.set_flags(CpuFlags::C, operand & 0x01 == 0x01);
    ...
    fn write_to_register(&mut self, opcode: u8, value: u8) -> u8{
        ...
        if destination_register == 0x06{
            ...
            self.registers.set_register_by_index(destination_register, value);
            return 0;
        }
